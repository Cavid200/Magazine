    ...
    <div class="container-fluid">
        <div class="page-title">
            <div class="row">
                ...
                <div class="col-6">
                    <a style="float: right" href="{{ route('admin.user.create') }}" class="btn btn-success">{{ __('create') }}</a>
                </div>
            </div>
        </div>
    </div>
                                ...
                                <tbody>
                                    @foreach ($users as $user)
                                        <tr>
                                            ...
                                            <td><img src="{{ asset($user->image) }}" alt="" style="width: 100px"></td>
                                            <td>{{ $user->fullname }}</td>
                                            <td>{{ $user->role->name }}</td>
                                            <td class="font-success">{{ $user->isActive == 1 ? __('active') : __('deactive')  }}</td>
                                            <td>
                                                ...
                                                @else
                                                    <a href="{{ route('admin.user.active', $user) }}"
